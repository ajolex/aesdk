...
Executable rule coverage now exists for every bundled method pack. AESDK ships 109 runnable governance rules across OLS/CEF, IV/2SLS, panel inference, DiD, randomized controlled trials/experimental methods, RDD, matching, synthetic control, nonlinear DiD, GMM, limited dependent variable models, time series, citation integrity, and AI replicability. The expanded methods still carry maturity labels where human econometrician review is pending, but they are no longer guidance-only.
